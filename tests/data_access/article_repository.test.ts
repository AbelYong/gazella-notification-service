...
suite("Article Repository", () => {
    ...
    describe("Get Unread Notifications", () => {
        test("Should retrieve only unread notifications for the specified addressee", async () => {
            ...
            await db.insert(ArticleNotifications).values([
                {
                    id: "11111111-1111-1111-1111-111111111111",
                    addresseeId: testAddresseeId,
                    messageBody: { message: "Unread notification 1" },
                    markedAsRead: false,
                    receivedAt: testDate
                },
                {
                    ...
                    addresseeId: testAddresseeId,
                    messageBody: { message: "Read notification 1" },
                    markedAsRead: true,
                    receivedAt: testDate
                },
                {
            ...
            expect(result[0]?.messageBody).toEqual({ message: "Unread notification 1" });
        });
        ...
        test("Should return an empty array if there are no unread notifications", async () => {
            await db.insert(ArticleNotifications).values({
                id: "11111111-1111-1111-1111-111111111111",
                addresseeId: testAddresseeId,
                messageBody: { message: "Already read" },
                markedAsRead: true
            });
